...
/** Count-up duration in ms — quick but still readable as the numbers climb. */
const COUNT_DUR = 2800;

// `n` values are fallback defaults; the live counts come from /content/stats.json
// (generated by build_content.py) and are matched in by `key`, so these cards
// never drift from the manuscript again. prerender-meta.cjs fails the build when
// these literals disagree with content/stats.json, because they did drift (2026-09-05).
const BOOK_STATS = [
  { key: "chapters", n: "21", label: "chapters", icon: "book-nav" },
  { key: "words", n: "136k", label: "words", icon: "note-nav" },
  { key: "figures", n: "50", label: "charts & figures", icon: "growth-nav" },
  { key: "receipts", n: "104", label: "tracked receipts", icon: "journal-nav" },
  { key: "links", n: "305", label: "linked sources", icon: "portal-nav" },
  { key: "glossary", n: "79", label: "glossary terms", icon: "chest-nav" },
];

const DEVICE_STATS = [
  { key: "goblinChecks", n: "27", label: "Goblin Checks", icon: "check-nav" },
  { key: "goblinTraps", n: "20", label: "Goblin Traps", icon: "trap-nav" },
  { key: "goblinFacts", n: "20", label: "Goblin Facts", icon: "potion-nav" },
  { key: "alignments", n: "21", label: "Alignments", icon: "guide-nav" },
  { key: "examples", n: "22", label: "Examples", icon: "examples-nav" },
];
    ...
    const io = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting && !done.current) {
        ...
        const step = (now: number) => {
          const p = Math.min(1, (now - t0) / COUNT_DUR);
...
/** Fixed-but-scattered sparkle positions (deterministic so they don't reshuffle). */
const SPARKLES = Array.from({ length: 28 }, (_, i) => ({
  left: 2 + ((i * 41) % 95),
  top: (i % 2 ? 56 : 1) + ((i * 23) % 40),
  size: 9 + ((i * 7) % 16),
  delay: (i * 83) % 1300,
  gold: i % 7 !== 0,
}));

/** A one-shot twinkle of sparkles over the stats grids. */
function Sparkles() {
  return (
    <div aria-hidden style={{ position: "absolute", inset: "-14px", pointerEvents: "none", zIndex: 4, overflow: "visible" }}>
      {SPARKLES.map((s, i) => (
        <span
          key={i}
          style={{
            ...
            fontSize: `${s.size}px`,
            lineHeight: 1,
            color: s.gold ? "#d9a441" : "#5a8f4e",
            textShadow: s.gold ? "0 0 7px rgba(217,164,65,0.6)" : "0 0 7px rgba(90,143,78,0.55)",
            animation: `dg-spark 3.4s ease-out ${s.delay}ms both`,
          }}
        >
          ✦
        </span>
...
/** Wraps the stat grids and fires the sparkle twinkle once, after the count-up lands. */
function StatsBurst({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting && !done.current) {
        ...
        timer = setTimeout(() => setBurst(true), COUNT_DUR + 200);
      }
    }, { threshold: 0.4 });
    io.observe(el);
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <style>{`@keyframes dg-spark{0%{transform:scale(0) rotate(0);opacity:0}14%{opacity:1}30%{transform:scale(1.2) rotate(18deg);opacity:1}72%{transform:scale(1) rotate(36deg);opacity:0.92}100%{transform:scale(0.25) rotate(54deg);opacity:0}}`}</style>
      {children}
      {burst && <Sparkles />}
    </div>
  );
...
export function AboutPage() {
  ...
  // Live stat counts generated by the content pipeline; falls back to the
  // hardcoded `n` defaults if the fetch fails.
  ...
  const statValue = (s: { key: string; n: string }) =>
    stats && stats[s.key] != null ? String(stats[s.key]) : s.n;
  ...
  return (
    <StaticPageShell maxWidth="1120px">
        ...
        {/* By the numbers — wrapped so a sparkle twinkle fires once the counts land */}
        <StatsBurst>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "10px", marginBottom: "16px" }}>
          {BOOK_STATS.map((s, i) => (
            <div key={i} style={{ background: cardBg, border: `1px solid ${border}`, borderRadius: "8px", padding: "16px 14px", textAlign: "center" }}>
              ...
              <div style={{ fontFamily: DISPLAY, fontSize: "30px", fontWeight: 900, color: green, lineHeight: 1 }}><CountUp value={statValue(s)} /></div>
