...
/** Count-up duration in ms (a touch slower so the numbers read as they climb). */
const COUNT_DUR = 3600;
...
const DEVICE_STATS = [
  ...
  { key: "goblinFacts", n: "20", label: "Goblin Facts", icon: "data-nav" },
  { key: "alignments", n: "20", label: "Alignments", icon: "guidebook-nav" },
      ...
      {SPARKLES.map((s, i) => (
        <span
          ...
          style={{
            ...
            animation: `dg-spark 1.5s ease-out ${s.delay}ms both`,
          }}
        >
          ✦
        </span>
...
function StatsBurst({ children }: { children: ReactNode }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <style>{`@keyframes dg-spark{0%{transform:scale(0) rotate(0);opacity:0}35%{opacity:1}55%{transform:scale(1.15) rotate(25deg);opacity:1}100%{transform:scale(0.2) rotate(50deg);opacity:0}}`}</style>
      {children}
      {burst && <Sparkles />}
    </div>
  );
