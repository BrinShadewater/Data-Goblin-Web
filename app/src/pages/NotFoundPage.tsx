  ...
  return (
    <StaticPageShell padding="48px clamp(16px, 5vw, 54px) 72px">
      ...
      <PageHeading
        ...
        titleLineHeight={1}
        description="The goblin checked its map, and this path isn&rsquo;t on it. The page may have moved, the link may be stale, or the URL may have a typo. No receipts were harmed."
      ...
      <StaticCard marginBottom="0" padding="18px 20px" background={c(...P.greenBg)} borderColor={c(...P.greenBorder)} borderLeft={`4px solid ${green}`}>
        ...
        <Link to="/" style={link}>Home</Link>{dot}
        <Link to="/guide" style={link}>Field guide</Link>{dot}
        <Link to="/map" style={link}>Map</Link>{dot}
        <Link to="/loot" style={link}>Glossary</Link>{dot}
        <Link to="/receipts" style={link}>Receipts</Link>
      </StaticCard>
    </StaticPageShell>
  );
}
